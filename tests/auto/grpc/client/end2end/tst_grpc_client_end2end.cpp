...
void QtGrpcClientEnd2EndTest::maximumReceiveMessageSize()
{
    ...
    QFETCH_GLOBAL(const QUrl, hostUri);
    QFETCH_GLOBAL(const QGrpcChannelOptions, channelOptions);
    QGrpcChannelOptions chOpts = channelOptions;
    if (channelLimit)
...
void QtGrpcClientEnd2EndTest::reconnectBackoff_data() const
{
    ...
    QTest::addColumn<const QGrpcChannelOptions>("opts");
    ...
    // By the fourth call the delay must exceed the initial backoff's range.
    {
        ...
        opts.setInitialReconnectBackoff(100ms).setMaximumReconnectBackoff(2000ms);
        QTest::newRow("delays-grow") << opts << noEnv << noEnv << 4 << 150 << -1 << noWarning;
    }
    // Uncapped growth would put the sixth call beyond 500ms; the maximum
    // keeps it near 200ms.
    {
        QGrpcChannelOptions opts;
        opts.setInitialReconnectBackoff(100ms).setMaximumReconnectBackoff(200ms);
        QTest::newRow("delays-capped-at-maximum")
            << opts << noEnv << noEnv << 6 << 100 << 500 << noWarning;
    }
    // Zero disables the backoff; retries stay immediate, far below the 1s
    // default that would apply if the zero were not honored.
    {
        QGrpcChannelOptions opts;
        opts.setInitialReconnectBackoff(0ms);
        QTest::newRow("zero-disables-backoff")
            << opts << noEnv << noEnv << 3 << 0 << 800 << noWarning;
    }
    // Even a 1ms initial backoff must keep growing into a measurable delay.
    {
        QGrpcChannelOptions opts;
        opts.setInitialReconnectBackoff(1ms).setMaximumReconnectBackoff(2000ms);
        QTest::newRow("one-ms-grows") << opts << noEnv << noEnv << 12 << 100 << -1 << noWarning;
    }
    // An initial backoff above the maximum warns and is clamped down to it.
    {
        QGrpcChannelOptions opts;
        opts.setInitialReconnectBackoff(500ms).setMaximumReconnectBackoff(100ms);
        QTest::newRow("initial-clamped-to-maximum")
            << opts << noEnv << noEnv << 2 << 60 << 400
            << QRegularExpression("Initial reconnect backoff of 500ms exceeds the maximum of "
                                  "100ms");
    }
    // With no option set the env value applies (default pacing would exceed
    // the bound).
    QTest::newRow("env-initial-applies")
        << QGrpcChannelOptions() << "0"_ba << noEnv << 2 << 0 << 800 << noWarning;
    // The env maximum applies: the default 1s initial clamps down to it.
    QTest::newRow("env-maximum-applies")
        << QGrpcChannelOptions() << noEnv << "0"_ba << 2 << 0 << 800
        << QRegularExpression("Initial reconnect backoff of 1000ms exceeds the maximum of 0ms");
    // An explicitly set option takes precedence over the environment.
    {
        QGrpcChannelOptions opts;
        opts.setInitialReconnectBackoff(0ms);
        QTest::newRow("option-beats-env")
            << opts << "60000"_ba << noEnv << 2 << 0 << 800 << noWarning;
    }
    // Invalid env values warn and are ignored.
    QTest::newRow("env-invalid-ignored")
        << QGrpcChannelOptions() << "-5"_ba << noEnv << 1 << 0 << -1
...
void QtGrpcClientEnd2EndTest::reconnectBackoff()
{
    ...
    QFETCH(const QGrpcChannelOptions, opts);
    ...
    QGrpcChannelOptions channelOpts = opts;
    channelOpts.setConnectTimeout(5s);

    qt::EventHub::Client client;
    QVERIFY(client.attachChannel(std::make_shared<QGrpcHttp2Channel>(url, channelOpts)));
    ...
    for (int i = 0; i < failingCalls; ++i) {
        ...
        QCOMPARE_EQ(qvariant_cast<QGrpcStatus>(spy.at(0).first()).code(),
                    StatusCode::Unavailable);
    }
...
void QtGrpcClientEnd2EndTest::reconnectRecoversAfterServerRestart()
{
    ...
    QFETCH_GLOBAL(const QGrpcChannelOptions, channelOptions);
    QGrpcChannelOptions opts = channelOptions;
    opts.setInitialReconnectBackoff(200ms).setMaximumReconnectBackoff(2000ms);
...
void QtGrpcClientEnd2EndTest::connectTimeout_data() const
{
    ...
    // Every timing-out row uses an effective timeout of 300ms; the warning
    // asserted below pins which source provided that value.
    {
        QGrpcChannelOptions opts;
        opts.setConnectTimeout(300ms);
        QTest::newRow("option-applies") << opts << QByteArray() << true;
    }
    QTest::newRow("env-applies") << QGrpcChannelOptions() << "300"_ba << true;
    {
        QGrpcChannelOptions opts;
        opts.setConnectTimeout(300ms);
        QTest::newRow("option-beats-env") << opts << "5000"_ba << true;
    }
    {
        QGrpcChannelOptions opts;
        opts.setConnectTimeout(0ms);
        QTest::newRow("zero-disables-timeout") << opts << "100"_ba << false;
    }
}
...
void QtGrpcClientEnd2EndTest::connectTimeout()
{
    ...
    qt::EventHub::Client client;
    QVERIFY(client.attachChannel(std::make_shared<QGrpcHttp2Channel>(url, opts)));
